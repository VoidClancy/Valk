...
// ProfileCreate contains model input fields for Profile creation operations.
//
// Fields for Profile:
//
...
// ProfileCreateArgs is the input argument passed to Profile Create extension hooks.
//
// Fields for Profile:
//
//	id        string    default: cuid()
...
// ProfileCreateManyArgs is the input argument passed to Profile CreateMany extension hooks.
//
// Fields for Profile:
//
//	id        string    default: cuid()
...
//	userId    string    required
//	createdAt time.Time default: now()
type ProfileCreateManyArgs struct {
...
//
//	user      (User)
type ProfileCreateManyAndReturnArgs struct {
	// Data is the slice of model inputs to bulk insert.
	Data []*ProfileCreate
...
func (cb *ProfileConflictBuilder[B]) Update(fn func(u *ProfileUpsert)) *B {
	cb.setAction(*ProfileConflictUpdate(fn), cb.conflictTarget)
	return cb.builder
}

// ProfileConflictUpdate creates a custom ConflictAction for Profile upsert conflicts.
func ProfileConflictUpdate(fn func(u *ProfileUpsert)) *ConflictAction {
